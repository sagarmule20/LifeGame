...
export default function QuestCard({ quest, tasks }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-xs mb-3" style={{ color: 'var(--text-dim)' }}>{quest.description}</p>
      <div className="flex items-center gap-2">
        <div className="flex-1 progress-track" style={{ height: 8 }}>
          <div className="progress-fill" style={{
            width: totalCount > 0 ? `${(completedCount / totalCount) * 100}%` : '0%',
            background: allDone ? 'var(--green)' : 'var(--blue)', height: 8,
          }} />
        </div>
        {earnedCoins > 0 && (
          <span className="text-xs font-extrabold" style={{ color: '#FFC800' }}>💶 +€{earnedCoins}</span>
        )}
      </div>
    </button>
  )
